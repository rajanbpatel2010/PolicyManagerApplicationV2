$filePath = "c:\Users\rajan.patel\Documents\RP_AG\PolicyManagerApp\backend\PolicyManager.API\Services\Services.cs"
$content = Get-Content $filePath -Raw

$startTag = "public async Task<ApiResponse<ExcelUploadResultDto>> UploadPoliciesFromExcelAsync(IFormFile file, int userId)"
$endTag = "public interface IPolicyTypeService"
        ...
        try
        {
            ...
            int FindCol(string[] names, int fallbackIdx) {
                foreach (var name in names) {
                    if (colDict.TryGetValue(name, out int idx)) return idx;
                }
                foreach(var col in colDict) {
                    if (names.Any(n => col.Key.Contains(n, StringComparison.OrdinalIgnoreCase))) return col.Value;
                }
                return fallbackIdx;
            }
            ...
            _logger.LogInformation("Excel Import - Mapping complete.");
            ...
            var defaultTypeId = policyTypes.FirstOrDefault(t => t.IsActive)?.Id ?? policyTypes[0].Id;
...
"@
    ...
    Write-Host "ERROR: Could not find start or end tags."
}
